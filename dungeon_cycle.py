...
from telegram_helpers import click_button, wait_update, resolve_warning
    ...
    async with client.conversation(BOT_USERNAME, timeout=RESPONSE_TIMEOUT) as conv:
        while True:
            ...
            if INJURY_STATUS_MARKER in profile_msg.raw_text:
                ...
                continue  # профиль/мощь могли измениться, начинаем виток заново
            ...
            # ---- 2. /dungeon -> список ----
            # Защита: если из-за незакрытого предыдущего диалога бот вместо
            # списка снова присылает предупреждение (энергия/HP) — чиним и повторяем.
            while True:
            ...
            if radar_msg is None:
                continue  # отчёт по старому данжу получен, начинаем виток заново с /profile

            entries = parse_dungeons(radar_msg.raw_text)
            resets_left = parse_resets_left(radar_msg.raw_text)
            log.info(
                f"Данжи: {[(e['idx'], e['name'], e['def']) for e in entries]} "
                f"| сбросов: {resets_left}"
            )

            # ---- 3. выбор данжа (с учётом сбросов) ----
            chosen = None
            while chosen is None:
                ...
                if chosen is not None:
                    break
                if not need_reset:
                    # не должно случиться (choose_dungeon либо вернёт данж, либо флаг reset)
                    break
            ...
            log.info(
                f"Выбран данж #{chosen['idx']} {chosen['name']} (DEF {chosen['def']})"
            )
            ...
            if not clicked:
                log.error(
                    f"Не нашёл кнопку 'Войти #{chosen['idx']}', пропускаю цикл."
                )
                continue

            # ВАЖНО: ответ на клик может прийти как edit того же сообщения,
            # а не как новое сообщение — поэтому ждём и то, и другое.
            ...
            if await resolve_warning(conv, enter_resp):
                # заново с шага 2 (/dungeon), НЕ дожидаясь отчёта — вход не состоялся
                continue
